...
public class SmartTerrainEventHandler : MonoBehaviour
{
    ...
    private SurfaceAbstractBehaviour mTrackedSurface;
    ...
    public Material SmartTerrainMat;
    ...
    public void OnSurfaceCreated(Surface surface)
    {
        ...
        if (mReconstructionBehaviour)
        {
            ...
            //}
            if (mReconstructionBehaviour.TryGetSurfaceBehaviour(surface, out mTrackedSurface))
                mTrackedSurface.gameObject.name = "Surface " + surface.ID;
        }
    }
    ...
    public void SwitchSurfaceMaterial()
    {
        if (mTrackedSurface)
			mTrackedSurface.gameObject.GetComponent<MeshRenderer>().material = SmartTerrainMat;
    }

	public void EnablePlayerFallDown()
	{
		if(mTrackedSurface.gameObject.GetComponent<PlayerFallDown>())
			mTrackedSurface.gameObject.GetComponent<PlayerFallDown>().enabled = true;
		else
			mTrackedSurface.gameObject.AddComponent<PlayerFallDown> ();
	}

    public void BuildNavMesh()
    {
		if (mAstar != null && !m_isScanning && !mAstar.isScanning)
        {
            ...
            foreach (NavGraph navGraph in mAstar.graphs)
            {
                if (navGraph is NavMeshGraph)
                {
                    ...
                    // set the source mesh
					mNavMesh = mTrackedSurface.Surface.GetNavMesh();
                    navMeshGraph.sourceMesh = mNavMesh;
